...
public class AdapterList2 extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    private List<ListElement> mData = new ArrayList<>();
    private List<ListElement> Original = new ArrayList<>();
    ...
    public AdapterList2(List<ListElement> itemList, String pEstado, Activity context, RecyclerView pRecycler, String pDisp, OnItemClickListener listener) {
    ...
    public void filtrarPorTexto(String texto) {
        ...
        for (int i = 0; i < Original.size(); i++) {
            ListElement p = Original.get(i);
            System.out.println("filtrar texto " + p.getPedido());
    ...
    private boolean contieneTexto(ListElement item, String texto) {
        String num = String.valueOf(item.getPedido()).toLowerCase();
    ...
    public boolean buscarPedido(int numP) {
        for (int i = 0; i < mData.size(); i++) {
            ListElement element = mData.get(i);
            if (element.getPedido() == numP) {
                return true;
            }
        }
        return false;
 ...
 */
    ...
    public void expandLessAll(ListElement item) {
        for (int i = 0; i < Original.size(); i++) {
            if (item.getPedido() != Original.get(i).getPedido()) {
        ...
        if (Original.size() > 1) {
            for (int i = 1; i < Original.size(); i++) {
                try {
                    ...
                    ListElement p = Original.get(i);
                    System.out.println("estadoActual " + p.getStatus());
                    System.out.println("atributo estadoActual = " + estadoActual);
                    if (estadoActual.equals(p.getStatus()) || p.getStatus().equals(getString(estadoActual))) {
                        if (p.getPrimera()) {
            ...
            if (status.equals(resources.getString(R.string.filtroOperativo))) {
                ...
                for (int i = 0; i < Original.size(); i++) {
                    ...
                    if (elemento.getStatus().equals(resources.getString(R.string.botonPendiente)) || elemento.getStatus().equals("PENDIENTE")) {
                        System.out.println("filtro operativo elemento " + elemento.getPedido());
    ...
    public class ViewHolder extends RecyclerView.ViewHolder {
        ...
        void bindData(final ListElement item, int position) {
            ...
            if (item.getEsPlaceHolder()) {
                ...
                numOrden.setText(resources.getString(R.string.num_pedido) + " " + item.getPedido());
                ...
                System.out.println("item " + item.getPedido() + " parpadeo " + item.getParpadeo());
                if (item.getStatus().equals(botonPendiente)) {
                    if (parpadeo) {
        ...
        private void setBarColor(ListElement item) {
            String est = item.getStatus();
